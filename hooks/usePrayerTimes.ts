...
export function usePrayerTimes(date?: Date): PrayerTimes | null {
  ...
  return useMemo(() => {
    ...
    return calculatePrayerTimes(coordinates, new Date(dateKey), method, madhab, prayerAdjustments)
  }, [coordinates, method, madhab, prayerAdjustments, dateKey])
}
...
export function useMultiDayPrayerTimes(days: number): DayPrayerTimes[] | null {
  ...
  return useMemo(() => {
    if (!coordinates) return null
    return buildDayPrayerTimes(coordinates, method, madhab, days, prayerAdjustments)
  }, [coordinates, method, madhab, prayerAdjustments, days, dateKey])
}
